...
cd apps 
su-exec frappe 'export PATH="${PATH}:/home/frappe/.local/bin"; pip3 install --no-cache-dir testfm'
su-exec frappe git clone --depth 1 -o upstream https://github.com/frappe/frappe 
su-exec frappe 'export PATH="${PATH}:/home/frappe/.local/bin"; pip3 install --no-cache-dir -e ${BENCH}/apps/frappe'
cd frappe 
su-exec frappe yarn
